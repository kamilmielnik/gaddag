import { LAST_ARC_FLAG, LETTER_MASK, MAGIC, SEPARATOR } from './constants.ts';
...
export class Gaddag {
  ...
  public static deserialize(bytes: Uint8Array): Gaddag {
    ...
    const expectedByteLength = 16 + 4 * (letterCount + arcCount) + arcCount;

    if (header[0] !== MAGIC || letterCount < 0 || arcCount < 1 || aligned.byteLength < expectedByteLength) {
      throw new Error('Invalid Gaddag data');
    }
